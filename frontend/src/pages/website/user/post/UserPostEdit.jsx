...
import { decParam, getCityState } from "@/utils/functions";
...
const UserPostEdit = () => {
  const { master, details, images } = useLoaderData();
  document.title = `${master.title} | ${import.meta.env.VITE_APP_TITLE}`;
  const remainingImg = 8 - images.length;
  ...
  const [form, setForm] = useState({
    catId: master.cat_id,
    price: Number(master.price).toFixed(0),
    city: master.city,
    state: master.state,
    address: "",
    isNew: master.is_new,
  });
  const [pinCode, setPinCode] = useState(master.pincode);
  ...
  const titleRemainingChar = 255 - char.title.length;
  const descRemainingChar = 500 - char.description.length;
  ...
  const deleteDbImage = async (id) => {
    setIsLoading(true);
    try {
      await customFetch.delete(`/users/posts/image/${id}`);
      navigate(pathname);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
  ...
  let dataArr = [];
  details.map((detail) => {
    dataArr.push({ fieldName: detail.field_name, fieldValue: detail.value });
  });

  useEffect(() => {
    if (master.cat_id) {
      const ch = allCategories
        .filter((i) => i.parent_id === Number(master.cat_id))
        .sort((a, b) => a.category.localeCompare(b.category));
      setChildren(ch);

      getFormFields(master.subcat_id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await customFetch.patch(`/users/posts/update/${postId}`, data);
      toast({
        title: "Updated!",
    ...
    <UserContentWrapper>
      ...
      >
          ...
          <div className="flex flex-col space-y-6 mt-0 p-4">
            <div className="flex flex-col">
              ...
              <span className="text-red-500 text-sm font-normal">
                ...
                {titleRemainingChar || remainingChar.title}
              </span>
            </div>
            ...
            <div className="flex flex-col">
              ...
              <span className="text-red-500 text-sm font-normal">
                Remaning characters:{" "}
                {descRemainingChar || remainingChar.description}
              </span>
            </div>
            <div className="flex flex-row gap-4">
              <div className="basis-1/2 flex flex-col">
        ...
        <div className="w-full rounded-xl flex flex-col space-y-6 bg-white">
          <div className="bg-purple-900 text-white rounded-xl rounded-b-none p-3 text-lg tracking-widest capitalize">
            Upload some cool images (max. 8 images) (remaing: {remainingImg})
          </div>
          <div className="flex flex-col space-y-6 mt-0 p-4">
            <div className="flex flex-row gap-4">
        ...
        <div className="w-full rounded-xl flex flex-row mt-6 justify-center items-center">
          <button
            type="submit"
            className="w-btn-secondary-lg border font-medium border-white px-6 py-3 text-md min-w-32 rounded-full capitalize tracking-wider"
            disabled={isLoading}
          >
            Update details
          </button>
...
export const loader =
  ...
  async ({ params }) => {
    ...
    const postId = decParam(id);
    try {
      const response = await customFetch.get(`/users/posts/edit/${postId}`);
      const master = response.data.master.rows[0];
      const details = response.data.details;
      const images = response.data.images.rows;
      return { master, details, images };
    } catch (error) {
      splitErrors(error?.response?.data?.msg);
      return redirect(`/user/${currentUser.slug}/posts`);
    }
  };
